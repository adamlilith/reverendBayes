---
title: "Multivariate Normal Regression in NIMBLE"
author: "Adam B. Smith"
date: "`r Sys.Date()`"
output:
  html_notebook:
...
This tutorial walks you through how to do implement a multipl-response regression where the responses are assumed to follow a multivariate normal distribution. The model is written in `nimble`.

## Setup
```{r, }
library(nimble) # the workhorse
library(coda) # for model diagnostics
library(ggmcmc) # plotting of MCMC
```
...
# define response of variable #1
beta1_0 <- 1 # intercept
beta1_1 <- 1.2 # slope
sigma1 <- 0.8 # variation around trendline

# define response of variable #2
beta2_0 <- 1.7 # intercept
beta2_1 <- -0.7 # slope
sigma2 <- 0.2 # variation around trendline

noise1 <- rnorm(N, mean = 0, sd = sigma1)
noise2 <- rnorm(N, mean = 0, sd = sigma2)
y1 <- beta1_0 + beta1_1 * x + noise1
y2 <- beta2_0 + beta2_1 * x + noise2
